...
class apply_attn(torch.autograd.Function):
    ...
    def forward(ctx, attn, value):
        ...
        if attn.is_cuda == True and value.is_cuda == True:
            ...
            n = output.nelement()
            cupy_launch('apply_attn_forward',
                        cupy_kernel('apply_attn_forward', intWindowSize, {
                            'attn': attn,
                            'value': value,
                            'output': output
                        }))(
    ...
    def backward(ctx, gradOutput):
        ...
        if attn.is_cuda == True and value.is_cuda == True:
            ...
            cupy_launch('apply_attn_backward',
                        cupy_kernel('apply_attn_backward', intWindowSize, {
                            'gradLoss': gradOutput,
                            'attn': attn,
                            'value': value,
                            'gradattn': gradattn,
